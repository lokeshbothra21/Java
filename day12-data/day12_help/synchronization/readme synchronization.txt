...
2. While using inherently thread un safe API(StringBuilder, ArrayList,LinkedList,HS,LHS,HM...) in multi threaded environment : you can still apply thread safety : using synchronized blocks.

synchronized block syntax

synchronized(shared resource ref./object under the lock)
{ 
Access the methods of shared resource in mutually exclusive manner(thread safe)
}
